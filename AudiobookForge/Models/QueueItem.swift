...
final class QueueItem: Identifiable {
    enum Status: Equatable {
        ...
        var isActive: Bool    { isPending || isRunning }
        var isFinished: Bool  { !isActive }
        var isRetryable: Bool { isFailed || isCancelled }
    ...
    var title: String { spec.metadata.title }
    var author: String { spec.metadata.author }
}
...
struct SourceFingerprint: Hashable {
    ...
    static func capture(_ url: URL,
                        fileManager: FileManager = .default) -> SourceFingerprint? {
        guard let attrs = try? fileManager.attributesOfItem(atPath: url.path),
              let size = (attrs[.size] as? NSNumber)?.int64Value,
              let mtime = attrs[.modificationDate] as? Date
        else { return nil }
        return SourceFingerprint(size: size, mtime: mtime)
    }
}
